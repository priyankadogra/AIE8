...
The agent analyzes user questions to determine intent (date queries, event lookups) and extracts relevant keywords and temporal information. The agent synthesizes information from multiple sources (structured events + unstructured documents) to provide coherent, accurate answers that combine both data types. The agent chooses appropriate tools based on query analysis - using event search for specific dates, RAG for general questions, and combining both for comprehensive answers.
...
I ended up manually creating my test data set as I was running into issues with the synthetic data generation. The error suggested that there might be some issue with generating personas. In the interest of time I switched to generating some questions myself and used it for both retrievals. Please refer to [table](evaluation/Ragas%20Evaluation%20Comparison.xlsx) [google docs link](https://docs.google.com/spreadsheets/d/163j1m4TgC1yK_oFrNCmWIbTJjzWlhJSQGZhuwWXSp1k/edit?usp=sharing) for Naive Retrieval metrics.
...
**Key insights from the above -** The answer generation is good. Both the precision and recall are not good. Low faithfulness suggests retrieved context is not sufficient for accurate answers.
...
Please refer to [table](evaluation/Ragas%20Evaluation%20Comparison.xlsx) [google docs link](https://docs.google.com/spreadsheets/d/163j1m4TgC1yK_oFrNCmWIbTJjzWlhJSQGZhuwWXSp1k/edit?usp=sharing) for Advanced Retrieval metrics using Cohere Re-ranking.
